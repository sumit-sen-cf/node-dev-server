...
exports.addDynamicTablesData = async (req, res) => {
    try {
        const check = await dynamicTablesModel.findOne({ table_name: req.body.table_name, user_id: req.body.user_id })
        if (check) {
            return response.returnFalse(400, req, res, 'Data allready exist for table name and user id', {})
        }
